...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['total_cancelled'] = Order::where('status', 'cancelled')->sum('cod_amount');

        $data['total_income'] = Order::all()->sum('cod_amount');

        $data['total_orders'] = Order::all()->count();

        $data['today_orders'] = Order::whereDate('created_at', Carbon::today())->where('status', 'delivered')->sum('cod_amount');
